...
#define MCL_IPW_TYPES_SW_PATCH_VERSION                4
...
/* Check if header file and Mcl_DmaMux_Types.h header file are of the same software version */
...
#if (MCL_ENABLE_FLEXIO == STD_ON)
    #if (MCL_IPW_TYPES_VENDOR_ID != FLEXIO_COMMON_TYPES_VENDOR_ID)
        #error "Mcl_IPW.h and Mcl_FlexIO_Common_Types.h have different vendor IDs"
    #endif
    /* Check if source file and Mcl_FlexIO_Common_Types header file are of the same Autosar version */
    #if ((MCL_IPW_TYPES_AR_RELEASE_MAJOR_VERSION != FLEXIO_COMMON_TYPES_AR_RELEASE_MAJOR_VERSION) || \
         (MCL_IPW_TYPES_AR_RELEASE_MINOR_VERSION != FLEXIO_COMMON_TYPES_AR_RELEASE_MINOR_VERSION) || \
         (MCL_IPW_TYPES_AR_RELEASE_REVISION_VERSION != FLEXIO_COMMON_TYPES_AR_RELEASE_REVISION_VERSION) \
        )
    #error "AutoSar Version Numbers of Mcl_IPW.h and Mcl_FlexIO_Common_Types.h are different"
    #endif
    /* Check if source file and Mcl_FlexIO_Common_Types header file are of the same Software version */
    #if ((MCL_IPW_TYPES_SW_MAJOR_VERSION != FLEXIO_COMMON_TYPES_SW_MAJOR_VERSION) || \
         (MCL_IPW_TYPES_SW_MINOR_VERSION != FLEXIO_COMMON_TYPES_SW_MINOR_VERSION) || \
         (MCL_IPW_TYPES_SW_PATCH_VERSION != FLEXIO_COMMON_TYPES_SW_PATCH_VERSION) \
        )
    #error "Software Version Numbers of Mcl_IPW.h and Mcl_FlexIO_Common_Types.h are different"
    #endif
#endif    

#if (MCL_SELECT_COMMON_TIMEBASE_API == STD_ON)    
    #if (MCL_IPW_TYPES_VENDOR_ID != FTM_COMMON_VENDOR_ID)
    ...
    #if ((MCL_IPW_TYPES_AR_RELEASE_MAJOR_VERSION != FTM_COMMON_AR_RELEASE_MAJOR_VERSION) || \
         (MCL_IPW_TYPES_AR_RELEASE_MINOR_VERSION != FTM_COMMON_AR_RELEASE_MINOR_VERSION) || \
         (MCL_IPW_TYPES_AR_RELEASE_REVISION_VERSION != FTM_COMMON_AR_RELEASE_REVISION_VERSION) \
        )
    ...
    #if ((MCL_IPW_TYPES_SW_MAJOR_VERSION != FTM_COMMON_SW_MAJOR_VERSION) || \
         (MCL_IPW_TYPES_SW_MINOR_VERSION != FTM_COMMON_SW_MINOR_VERSION) || \
         (MCL_IPW_TYPES_SW_PATCH_VERSION != FTM_COMMON_SW_PATCH_VERSION) \
        )

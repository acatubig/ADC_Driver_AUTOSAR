...
#define ADC_SW_PATCH_VERSION_PDB_IRQ_C             4

/*==================================================================================================
                                      FILE VERSION CHECKS
==================================================================================================*/
/* Check if source file and Adc_Pdb header file are of the same vendor */
#if (ADC_AR_VENDOR_ID_PDB_IRQ_C != ADC_VENDOR_ID_PDB)
...
)
{
    ...
    for (u8PDBChannelCount = 0U; u8PDBChannelCount < ADC_PDB_CHANNELS_U8; u8PDBChannelCount++)
    {
        ...
        if(REG_READ32(PDB_CH_N_S_REG_ADDR32(u8Unit, u8PDBChannelCount)) & PDB_ERR_FLAGS_MASK_U32)
        {
            ...
            /** @violates @ref Adc_Pdb_Irq_c_REF_3 cast from unsigned long to pointer */
            /** @violates @ref Adc_Pdb_Irq_c_REF_5 A cast should not be performed between a pointer type and an integral type. */
            REG_WRITE32(PDB_CH_N_S_REG_ADDR32(u8Unit, u8PDBChannelCount), PDB_ERR_FLAGS_CLEAR_U32);
...
)
{
    ...
    if((uint8)STD_ON == Adc_pCfgPtr->pGroups[Group].u8AdcGroupChannelDelays)
    {
        for(u8ScRegister = 0U; u8ScRegister < Adc_aUnitStatus[u8Unit].u8Sc1Used; u8ScRegister++)
        {
            ...
            /** @violates @ref Adc_Pdb_Irq_c_REF_3 cast from unsigned long to pointer */
            /** @violates @ref Adc_Pdb_Irq_c_REF_5 A cast should not be performed between a pointer type and an integral type. */
            (void)REG_READ32(ADC12BSARV2_R_N_REG_ADDR32(u8Unit, (uint32)u8ScRegister));
        }        
    }
    ...
    {
        /* PR-MCAL-3324: DEM calls must not happen in interrupt context. */
        /** @violates @ref Adc_Pdb_Irq_c_REF_9 Return statement before end of function. */
        return;
    }
    else
...
ISR(Adc_Pdb_ChannelSequenceError0)
{
    Adc_Pdb_ChannelSequenceError((uint8)ADC_UNIT_0);
}
#endif
...
ISR(Adc_Pdb_ChannelSequenceError1)
{
    Adc_Pdb_ChannelSequenceError((uint8)ADC_UNIT_1);
}
#endif
